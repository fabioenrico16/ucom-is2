...
public class Rest extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		.setExchangePattern(ExchangePattern.InOnly) 
			.choice()
			.when(header("BANCO_DESTINO").contains("ATLAS")).to("activemq:Acosta-ATLAS-IN").endChoice()
			.when(header("BANCO_DESTINO").contains("ITAU")).to("activemq:Acosta-ITAU-IN").endChoice()
			.otherwise()
